...
ABS_PATH = os.path.dirname(os.path.abspath(__file__)) + "/examples"


class JavaExample:
    with open(f"{ABS_PATH}/1.java", "r") as f:
        EXAMPLE_CODE1 = f.read()

    with open(f"{ABS_PATH}/1_spec.java", "r") as f:
        EXAMPLE_SPEC1 = f.read()
        EXAMPLE_SPEC1 = "```\n" + EXAMPLE_SPEC1 + "\n```\n"

    with open(f"{ABS_PATH}/2.java", "r") as f:
        EXAMPLE_CODE2 = f.read()

    with open(f"{ABS_PATH}/2_spec.java", "r") as f:
...
"""
    ...
    with open(f"{ABS_PATH}/3.java", "r") as f:
        EXAMPLE_CODE3 = f.read()

    with open(f"{ABS_PATH}/3_spec.java", "r") as f:
...
_GUIDANCE = {
    ...
    "SyntaxError": {"description": "", "guidance": _SYNTAX_ERROR_GUIDANCE},
    "LargeShift": {"description": _LARGE_SHIFT_DESC, "guidance": _LARGE_SHIFT_GUIDANCE},
    "PreconditionFailure": {
        "description": _PRECONDITION_DESC,
        "guidance": _PRECONDITION_GUIDANCE,
    },
    "BadCast": {"description": _BAD_CAST_DESC, "guidance": _BAD_CAST_GUIDANCE},
}
